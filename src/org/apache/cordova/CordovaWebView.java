...
//            App, LOG, CordovaChromeClient, CordovaBridge, 
//            NativeToJsMessageQueue, CordovaResourceApi, PluginEntry, Whitelist, 
//            CordovaWebViewClient, IceCreamCordovaWebViewClient, ScrollEvent, CordovaPreferences, 
//            PluginResult
...
public class CordovaWebView extends WebView
{
    ...
    private static final class Level17Apis
    {
        ...
        static void setMediaPlaybackRequiresUserGesture(WebSettings websettings, boolean flag)
        {
    ...
    private void initWebViewSettings()
    {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 17)
        {
            Level17Apis.setMediaPlaybackRequiresUserGesture(((WebSettings) (obj)), false);
        }
    ...
    private void sendJavascriptEvent(String s)
    {
        if (appPlugin == null)
        {
            appPlugin = (App)pluginManager.getPlugin("App");
        }
        if (appPlugin == null)
        {
            LOG.w("CordovaWebView", "Unable to fire event without existing plugin");
            return;
        } else
        {
    ...
    {
        loadUrlTimeout = loadUrlTimeout + 1;
        loadUrl("about:blank");
        chromeClient.destroyLastDialog();
        if (pluginManager != null)
        {
    ...
    public void init(CordovaInterface cordovainterface, CordovaWebViewClient cordovawebviewclient, CordovaChromeClient cordovachromeclient, List list, Whitelist whitelist, Whitelist whitelist1, CordovaPreferences cordovapreferences)
    {
        ...
        } else
        {
            ...
            bridge = new CordovaBridge(pluginManager, new NativeToJsMessageQueue(this, cordovainterface), cordova.getActivity().getPackageName());
...
_L1:
    ...
    void loadUrlNow(String s)
    {
        ...
        if (s.startsWith("file://") || s.startsWith("javascript:") || s.startsWith("about:") || internalWhitelist.isUrlWhiteListed(s))
        {
            super.loadUrl(s);
        }
    }
