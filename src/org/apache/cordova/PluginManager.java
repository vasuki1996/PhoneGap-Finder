...
//            ICordovaClientCertRequest, ICordovaHttpAuthHandler
...
label0:
    ...
    public boolean onReceivedHttpAuthRequest(CordovaWebView cordovawebview, ICordovaHttpAuthHandler icordovahttpauthhandler, String s, String s1)
    {
        for (Iterator iterator = pluginMap.values().iterator(); iterator.hasNext();)
        {
            CordovaPlugin cordovaplugin = (CordovaPlugin)iterator.next();
            if (cordovaplugin != null && cordovaplugin.onReceivedHttpAuthRequest(cordovawebview, icordovahttpauthhandler, s, s1))
            {
                return true;
            }
        }
